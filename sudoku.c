...
int generatesudoku(int box[9][9],int i,int j){
    if(i==8 && j==9)  return 1;
    if(j==9)  return generatesudoku(box,i+1,0);
    if(box[i][j]!=0) return generatesudoku(box,i,j+1);
    int check[11];
